let
  ...
  big-fm = name: "https://streams.bigfm.de/bigfm-${name}-128-aac?usid=0-0-H-A-D-01";
...
in
{
  ...
  i18n = m3u [ # Country and Culture Specific Music
    "http://62.210.24.124:8379/;stream.mp3 " # Gjirokastër
    "http://iphone.live24.gr/derty1000" # derti – μόνο λαϊκά
    "https://knr.gl/radiolivestream" # kalaallit nunaata radioa
    (big-fm "balkan")
    (big-fm "latinbeats")
    (big-fm "orient")
    (big-fm "russia")
  ...
  Out-There = m3u [
    "http://klassikr.streamabc.net/klassikradio-simulcast-mp3-hq" # Klassikradio
  ...
  Charts = m3u [
    "http://185.80.220.12:8166/stream" # "Raidió Rírá"
    (big-fm "charts")
    (big-fm "deutschland")
    (big-fm "mashup")
    (rte "2fm")
